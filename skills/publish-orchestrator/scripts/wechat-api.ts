...
interface PublishStatusResponse {
  publish_id?: string;
  publish_status?: number; // 0=success, 1=publishing, 2+=failed
  article_id?: string;
  article_detail?: {
    count: number;
    item: Array<{
...
function loadJsonConfig(filePath: string): { appId?: string; appSecret?: string } {
  try {
    const content = fs.readFileSync(filePath, "utf-8");
    const data = JSON.parse(content);
    return {
      appId: data.appid || data.appId || data.WECHAT_APP_ID,
      appSecret: data.secret || data.appsecret || data.appSecret || data.WECHAT_APP_SECRET,
    };
  } catch {
    return {};
  }
...
function getSecretsCandidates(): string[] {
  const candidates = [
    path.join(getProjectRoot(), ".secrets", "wechat-config.json"),
    path.join(process.cwd(), ".secrets", "wechat-config.json"),
  ];
...
function loadConfig(): WechatConfig {
  // Priority 1: project secrets file(s)
  let appId: string | undefined;
  let appSecret: string | undefined;

  for (const secretsPath of getSecretsCandidates()) {
    const secrets = loadJsonConfig(secretsPath);
    appId = appId || secrets.appId;
    appSecret = appSecret || secrets.appSecret;
    if (appId && appSecret) {
      break;
    }
  }
  ...
  // Priority 3: Legacy .baoyu-skills/.env (backward compatibility)
  if (!appId || !appSecret) {
    ...
    appId = appId || cwdEnv.WECHAT_APP_ID || homeEnv.WECHAT_APP_ID;
    appSecret = appSecret || cwdEnv.WECHAT_APP_SECRET || homeEnv.WECHAT_APP_SECRET;
  }

  if (!appId || !appSecret) {
    throw new Error(
      "Missing WeChat AppID/AppSecret.\n" +
      `Checked project secrets: ${getSecretsCandidates().join(", ")}\n` +
      "Also checked environment variables (WECHAT_APP_ID, WECHAT_APP_SECRET) and legacy .baoyu-skills/.env"
    );
  }
...
function renderMarkdownToHtml(markdownPath: string, theme: string = "default", title?: string): string {
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = path.dirname(__filename);
  const renderScript = path.join(__dirname, "md", "render.ts");
  const baseDir = path.dirname(markdownPath);
  const command = ["-y", "bun", renderScript, markdownPath, "--theme", theme];
...
Options:
  ...
  --theme <name>      Theme name for markdown (default, simple). Default: default
  --cover <path>      Cover image path (local or URL)
  --publish           Publish via freepublish after saving to draft
...
Config File Locations (in priority order):
  1. <project-root>/.secrets/wechat-config.json (appid, secret)
  2. <cwd>/.secrets/wechat-config.json (appid, secret)
  3. Environment variables (WECHAT_APP_ID, WECHAT_APP_SECRET)
  4. <cwd>/.baoyu-skills/.env (legacy fallback)

Example:
  npx -y bun wechat-api.ts article.md
  npx -y bun wechat-api.ts article.md --theme default --cover cover.png
...
const CONTENT_IMAGE_URL = "https://api.weixin.qq.com/cgi-bin/media/uploadimg";
const FREEPUBLISH_URL = "https://api.weixin.qq.com/cgi-bin/freepublish/submit";
const PUBLISH_STATUS_URL = "https://api.weixin.qq.com/cgi-bin/freepublish/get";
...
async function uploadContentImage(
  ...
  if (!fs.existsSync(resolvedPath)) {
    throw new Error(`Content image not found: ${resolvedPath}`);
  }
  fileBuffer = fs.readFileSync(resolvedPath);
  filename = path.basename(resolvedPath);
  const ext = path.extname(filename).toLowerCase();
  ...
  const url = `${CONTENT_IMAGE_URL}?access_token=${accessToken}`;
  const res = await fetch(url, {
    method: "POST",
    headers: {
      "Content-Type": `multipart/form-data; boundary=${boundary}`,
  ...
  if (!data.url) {
    throw new Error(`Content image upload failed: ${data.errcode} - ${data.errmsg}`);
  }

  console.error(`[wechat-api] Content image uploaded: ${data.url.slice(0, 80)}...`);
  return data.url;
}
...
async function freepublish(
  mediaId: string,
  accessToken: string
): Promise<FreepublishResponse> {
  const url = `${FREEPUBLISH_URL}?access_token=${accessToken}`;
  const res = await fetch(url, {
    method: "POST",
  ...
  const data = await res.json() as FreepublishResponse;
  if (data.errcode && data.errcode !== 0) {
    throw new Error(`Freepublish failed ${data.errcode}: ${data.errmsg}`);
  }

  console.error(`[wechat-api] Freepublish submitted, publish_id: ${data.publish_id}`);
  return data;
}
...
async function getPublishStatus(
  ...
  const url = `${PUBLISH_STATUS_URL}?access_token=${accessToken}`;
  const res = await fetch(url, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  return await res.json() as PublishStatusResponse;
}

async function publishAndPollStatus(
  mediaId: string,
  accessToken: string,
  maxRetries: number = 6,
  intervalMs: number = 5000
): Promise<PublishStatusResponse> {
  const pubResult = await freepublish(mediaId, accessToken);
  const publishId = pubResult.publish_id;
  if (!publishId) {
    throw new Error("No publish_id returned from freepublish");
  }

  console.error(`[wechat-api] Polling publish status (publish_id: ${publishId})...`);

  for (let i = 0; i < maxRetries; i++) {
    await new Promise(resolve => setTimeout(resolve, intervalMs));
    const status = await getPublishStatus(publishId, accessToken);

    if (status.publish_status === 0) {
      console.error(`[wechat-api] Publish succeeded!`);
      if (status.article_detail?.item?.[0]?.article_url) {
        console.error(`[wechat-api] Article URL: ${status.article_detail.item[0].article_url}`);
      }
      return status;
    } else if (status.publish_status !== undefined && status.publish_status > 1) {
      throw new Error(`Publish failed with status ${status.publish_status}: ${JSON.stringify(status)}`);
    }

    console.error(`[wechat-api] Still publishing... (attempt ${i + 1}/${maxRetries})`);
  }

  console.error(`[wechat-api] Publish still in progress after ${maxRetries} polls`);
  return await getPublishStatus(publishId!, accessToken);
}
...
async function main(): Promise<void> {
  ...
  if (args.isHtml) {
    ...
    htmlPath = renderMarkdownToHtml(filePath, args.theme, title);
  ...
  if (args.publish && result.media_id) {
    console.error("[wechat-api] Publishing via freepublish...");
    const pubStatus = await publishAndPollStatus(result.media_id, accessToken);
    console.log(JSON.stringify({
      success: true,
      media_id: result.media_id,
      title,
