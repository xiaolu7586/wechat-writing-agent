...
Usage:
    ...
    # 指定尺寸
    python3 generate_image.py --prompt "数据图表" --size 1280*720 -o chart.jpg
...
DEFAULT_OUTPUT_DIR = Path(__file__).parent.parent / "output" / "images"
SCRIPT_DIR = Path(__file__).parent
WORKSPACE_ROOT = SCRIPT_DIR.parent.parent.parent.parent  # skills/image-generator/scripts -> agent root
...
def main():
    parser = argparse.ArgumentParser(
        description="文章插图生成器（Seedream 5.0 Lite）"
    )
    ...
    try:
        ...
        if h >= w:
            print(f"[Warning] 尺寸 {args.size} 不是宽图，自动转为默认宽图 1280*720")
            args.size = "1280*720"
    except ValueError:
        pass
    ...
    # 尝试 AI 生成
    success = generate_ai(
        prompt, args.style, args.size, output_path,
